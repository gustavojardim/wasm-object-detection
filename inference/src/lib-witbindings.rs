...
use exports::wasi::nn::graph::{self as graph_mod, ExecutionTarget, GraphEncoding};
use exports::wasi::nn::tensor::TensorType;
use exports::wasi::nn::inference;
...
fn load_graph(model_name: &str) -> Result<graph_mod::Graph> {
    let model_path = format!("/models/{}", model_name);
    let model_bytes = std::fs::read(&model_path)
        .with_context(|| format!("failed to read model at {}", model_path))?;
    
    let builder = graph_mod::GraphBuilder::new(
        GraphEncoding::Pytorch,
        ExecutionTarget::Gpu,
    );
    
    // Try GPU first, fallback to CPU
    builder.build_from_bytes(&[&model_bytes])
        .or_else(|_| {
            eprintln!("[INFO] GPU load failed, falling back to CPU...");
            let cpu_builder = graph_mod::GraphBuilder::new(
                GraphEncoding::Pytorch,
                ExecutionTarget::Cpu,
            );
            cpu_builder.build_from_bytes(&[&model_bytes])
        })
...
fn handle_client(mut stream: TcpStream, graph: &graph_mod::Graph) -> Result<()> {
    ...
    loop {
        ...
        let dims = vec![1, 3, TARGET_SIZE as u32, TARGET_SIZE as u32];
        let tensor = inference::Tensor {
            dimensions: dims,
        ...
        // Get output
        let output_buffer = ctx.get_output(0)?;
        let output_floats: Vec<f32> = output_buffer
            .data
            .chunks_exact(4)
